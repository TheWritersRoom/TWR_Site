...
export default function VideoWithControls() {
  ...
  // Export path — clean full-screen portrait, no controls
  if (!isIframed) {
    return (
      <div style={{
        width: '100vw', height: '100vh', overflow: 'hidden',
        background: '#1A1614', display: 'flex', alignItems: 'center', justifyContent: 'center',
      }}>
        <div style={{ height: '100vh', aspectRatio: '9/16', position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
          <VideoTemplate />
        </div>
      </div>
    );
  ...
  // Preview path — portrait container at 82vh + control bar
  return (
    <div style={{
      position: 'fixed', inset: 0, background: '#1A1614',
      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
    }}>
      {/* Portrait video at 82vh */}
      <div style={{ height: '82vh', aspectRatio: '9/16', position: 'relative', flexShrink: 0, overflow: 'hidden', borderRadius: '2vh' }}>
        <VideoTemplate
          key={mountKey}
          durations={durations}
          loop
      ...
      {/* Control bar sensor (spans full width) */}
      <div
        ref={sensorRef}
